<template>
  <div>
    ...
    <el-dialog v-model="dialogVisible" title="预览公告" width="50%">
      <div>
...
const handleSwitchChange = async (item) => {
  // console.log(item)
  await axios.put(`/adminapi/notice/publish`, {
    _id: item._id,
...
const handleDelete =async (item)=>{
    // console.log(item)
    await axios.delete(`/adminapi/notice/list/${item._id}`)
    await getTableData()
}

//编辑回调
const handleEdit = (item)=>{
    //跳转到/notice-manage/editnotice/:id

    router.push(`/notice-manage/editnotice/${item._id}`)
}
</script>
